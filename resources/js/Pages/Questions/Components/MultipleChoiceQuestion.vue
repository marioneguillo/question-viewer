    ...
    <template #options>
      <label v-for="option in question.options" :key="option.id" 
           ...
           :class="{
             ...
             'bg-red-50 border-red-200': showAnswers && !option.is_correct && selectedOptions.includes(option.id),
             'hover:bg-gray-50': !showAnswers,
             'bg-blue-50 border-blue-300 shadow-sm': !showAnswers && selectedOptions.includes(option.id)
           }">
        <div class="flex items-start">
          ...
          <span class="flex-grow" :class="{
            ...
            'text-red-700': showAnswers && !option.is_correct && selectedOptions.includes(option.id),
            'text-gray-700': !showAnswers && !selectedOptions.includes(option.id),
            'text-blue-700 font-medium': !showAnswers && selectedOptions.includes(option.id)
          }">{{ option.text }}</span>
        </div>
        <div v-if="showAnswers" class="mt-2 text-sm">
          ...
          <template v-else-if="selectedOptions.includes(option.id)">
